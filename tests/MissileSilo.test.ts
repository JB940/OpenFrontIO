...
describe("MissileSilo", () => {
  beforeEach(async () => {
    ...
    constructionExecution(game, attacker, 1, 1, "MissileSilo");
  });
  ...
  test("missilesilo should cooldown as long as configured", async () => {
    expect(attacker.units("MissileSilo")[0].isInCooldown()).toBeFalsy();
  ...
  test("missilesilo should have increased level after upgrade", async () => {
    expect(attacker.units("MissileSilo")[0].level()).toEqual(1);

    const upgradeStructureExecution = new UpgradeStructureExecution(
      attacker,
      attacker.units("MissileSilo")[0].id(),
    );
    game.addExecution(upgradeStructureExecution);
    executeTicks(game, 2);

    expect(attacker.units("MissileSilo")[0].level()).toEqual(2);
  });
});
